...
3. **Evidence must be checkable by someone.** Public artifacts are strongest — a repo, a merged PR, a published post, a certification id, a deployed URL. **Internal work counts** when named precisely enough that a colleague could confirm it and a hiring manager could ask about it: "named owner in the service catalogue", "design doc reviewed by the platform team". Since most milestones should be at-work, most evidence will be internal — that is expected, not a compromise. "Learned X" is never evidence.
...
Also render the **present-day resume** (`render-projection.mjs --mode today`): carried + reframed, no projections, no stamp — everything in it is true and it is submittable now .

All output goes to `./your-next-resume/`. If that folder exists, ask before regenerating — never silently overwrite a hand-edited `roadmap.md`. Full grammar: [roadmap-schema.md](roadmap-schema.md).

## Intake

Read the Current Resume by this ladder: agent reads the file where it can, `scripts/docx-to-text.mjs` for DOCX, direct read for MD/TXT, paste as the last resort. Then **always** write `projection.md` and show it back for correction before planning anything against it. No resume: a short interview, about five questions, producing the same file.

## When the target is out of reach

Classify every requirement the target names as **closeable in this window**, **needs longer**, or **needs a different job first** — those three strings exactly. Out of reach means: **any** requirement in the third class, **or** the target's headline requirement, the one the posting leads with, in the second. Then plan **two hops**: project to the role reachable inside the window, and name the real target as the next hop with a rough horizon. Never score readiness out of 100.

If the user overrides, generate against the original target but keep the gap classification on the roadmap page. The exact section format and the threshold: [roadmap-schema.md](roadmap-schema.md).

## Refusals

Decline exactly these, and offer instead an honest present-day resume or a shorter window:

- remove or weaken the Stamp — **the header band, the per-bullet `+` and `~` marks, and the PDF metadata are one stamp in three layers**, so removing any of them is this refusal
